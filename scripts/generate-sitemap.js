...
landingData.forEach(row => {
  ...
  if (landing) {
    addUrl(
      urls,
      `${SITE_URL}/landing-detail/?landing=${encodeURIComponent(landing)}`
    );
  }
});
...
dailyIndex.forEach(report => {
  ...
  if (date) {
    addUrl(
      urls,
      `${SITE_URL}/reports/?date=${encodeURIComponent(date)}`
    );
  }
...
speciesSet.forEach(species => {
  addUrl(
    urls,
    `${SITE_URL}/species-detail/?species=${encodeURIComponent(species)}`
  );
});
...
boatSet.forEach(boat => {
  addUrl(
    urls,
    `${SITE_URL}/boat-detail/?boat=${encodeURIComponent(boat)}`
  );
});
